...
<div class="card">
    <div class="card-header card-header-primary">
        <h4 class="card-title"><strong> Add A New ClickBank Product
        ...
        <table class="table">
            <tbody>
                ...
                <tr>
                    <th width="20%"></th>
                    <td>
                        @if($store->affiliateSettings->where('name', 'clickbank')->first() !== null && json_decode($store->affiliateSettings->where('name', 'clickbank')->first()->settings)->account_name !== '')
                        <button type="button" class="btn btn-primary search"> Search </button>
                        @else
                        <button type="button" class="btn btn-primary search" disabled> Search </button> 
                        <p class="text-danger"><b>Please enter Account Name for ClickBank Settings. Go to affiliate settings to add ClickBank Account Name</b></p>
                        @endif
                    </th>
                </tr>
            </tbody>
                ...
                <table class="table member-table text-center" id="product-list">
                    <thead class="">
                        <tr>
                            ...
                            <th>Gravity</th>
                            <th>Percent Per Sale</th>
                            <th>Percent Per Rebill</th>
                            <th>Commission</th>
                            <th>Avg. Earnings Per Sale</th>
...
@section('custom-scripts')
    ...
    <script>
        ...
        $('.search').on('click', function(){
            ...
            axios.post("{{ route('clickbank.search', ['subdomain' => Session::get('subdomain')]) }}", {
                keyword: keyword,
            })
            .then(function (responses) {
                ...
                html += '   <th>Popularity Rank</th>'
                html += '   <th>Gravity</th>'
                html += '   <th>Percent Per Sale</th>'
                html += '   <th>Percent Per Bill</th>'
                html += '   <th>Commission</th>'
                html += '   <th>Avg. Earnings Per Sale</th>'
                ...
                $.each(responses.data, function(i, item){
                        ...
                        var data = {
                            ...
                            price: 0.00,
                            details_link: '#',
                            description: escape(item.description.toString()) ,
                            customer_review_link: '',
                            images: [],
                            permalink: createPermalink(item.title),
                        };
                        ...
                        html += "   <td><input class='product-data' type='hidden' value='" + data + "'>"+ item.title +"</td>";
                        html += '   <td>'+ item.popularity_rank +'</td>';
                        html += '   <td>'+ item.gravity +'</td>';
                        html += '   <td>'+ item.percent_per_sale +'</td>';
                        html += '   <td>'+ item.percent_per_rebill +'</td>';
                        html += '   <td>'+ item.commission +'</td>';
                        html += '   <td>'+ item.average_earnings_per_sale +'</td>';
