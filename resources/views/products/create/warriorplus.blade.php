...
@section('page_title')
Warrior Plus Product
@endsection

@section('content')
<div class="card">
    <div class="card-header card-header-primary">
        <h4 class="card-title"><strong> Add A New Warrior Plus Product <i class="material-icons tip" data-toggle="tooltip" data-placement="right" title="To affiliate Warrior Plus Products you must need first to get approved by the vendor of the product.">info</i></strong></h4>
...
@section('custom-scripts')
    ...
    <script>
        ...
        $('.search').on('click', function(){
            ...
            axios.post("{{ route('warriorplus.search', ['subdomain' => Session::get('subdomain')]) }}", {
                keyword: keyword,
            })
            .then(function (responses) {
                ...
                html += '   <tr>';
                html += '       <th></th>';
                html += '       <th>Offer Code</th>';
                html += '       <th>Product Name</th>';
                html += '       <th>Vendor Name</th>';
                html += '       <th>Sales Range</th>';
                html += '       <th>Conversion Rate</th>';
                html += '       <th>Pulse Score</th>';
                ...
                $.each(responses.data, function(i, item){
                        ...
                        var data = {
                            title: escape(item.offer_name),
                            image: image,
                            product_id: item.offer_code,
                            currency: 'USD',
                            price: 0.00,
                            offer_url: escape(item.offer_url),
                            details_link: item.request_url,
                            description: '',
                            customer_review_link: '',
                            images: [],
                            permalink: createPermalink(item.offer_name),
                        };
                        ...
                        html += '<tr>';
                        html += '   <td></td>';
                        html += '   <td class="item-id">'+ item.offer_code +'</td>';
                        html += "   <td><input class='product-data' type='hidden' value='" + data + "'>"+ item.offer_name +"</td>";
                        html += '   <td>'+ item.vendor_name +'</td>';
                        html += '   <td>'+ item.sales_range +'</td>';
                        html += '   <td>'+ item.conv_rate +'</td>';
                        html += '   <td>'+ item.pulse_score +'</td>';
                        html += '   <td><a href="'+ item.request_url.replace('http://', 'https://') +'" target="_blank">View Details</a></td>';
                        html += '   <td> ' + radioBtns(item.offer_code) + ' </td>';
                        html += '</tr>';
