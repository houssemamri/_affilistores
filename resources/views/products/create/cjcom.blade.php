...
<div class="card">
    <div class="card-header card-header-primary">
        <h4 class="card-title"><strong> Add A New Cj.com Product </strong></h4>
...
@section('custom-scripts')
    ...
    <script>
        function getImage(product){
            ...
            if(product['image-url'] && product['image-url'].length > 0){
                image = product['image-url'].replace('http:', 'https:');
            }
        ...
        function getPrice(product){
            ...
            if(product['sale-price'] && product['sale-price'].length > 0 && product['sale-price'] > 0){
                price = product['sale-price'];
            // }else if(product['retail-price'] && product['retail-price'].length > 0){
            //     price = product['retail-price'] < product['price'] ? product['retail-price'] : product['price'];
            }else{
                price = product['price'];
            }
        ...
        function brokenImg(element){
            if($("#product-list")){
                var pTable = $("#product-list").DataTable();
                pTable.row( $(element).parents('tr') ).remove().draw();
            }
        }
        ...
        $('.search').on('click', function(){
            ...
            axios.post("{{ route('cjcom.search', ['subdomain' => Session::get('subdomain')]) }}", {
                keyword: keyword,
                ...
                if(responses.data.error){
                    ...
                    return;
                }else if(responses.data.products['@attributes']['records-returned'] == 0){
                ...
                var products = responses.data.products.product;
                ...
                $.each(products, function(index, product){
                    ...
                    var data = {
                        ...
                        price: price,
                        details_link: escape(product['buy-url']),
                        description: (product.description) ? escape(product.description.toString()) : '',
                        images: [],
                        permalink: createPermalink(product.name),
                    };
                  
                    console.log(data);
                    var display_price = price > 0 ? price : '<a href="'+ product['buy-url'] +'" target="_blank">Check Price</a>';
                    data = JSON.stringify(data);

                    html += '<tr>';
                    html += '   <td></td>';
                    html += '   <td>'+ product['catalog-id'] +'</td>';
                    html += "   <td><input class='product-data' type='hidden' value='"+ data +"'>"+ product.name +"</td>";
                    html += '   <td><img onerror="brokenImg(this)" src="'+ image +'" width="60%" alt="'+ product.name +'" class="img-fluid"></td>';
                    html += '   <td>' + product.currency +' '+ price +'</td>';
                    html += '   <td><a href="'+ product['buy-url'] +'" target="_blank">View Details</a></td>';
                    html += '</tr>';
                ...
                //     var checkbox = $(this)
                //     var image = checkbox.parents('tr').find('td > img')
