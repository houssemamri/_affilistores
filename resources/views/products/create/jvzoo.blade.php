...
<div class="card">
    <div class="card-header card-header-primary">
        <h4 class="card-title"><strong> Add A New JVZoo Product  <i class="material-icons tip" data-toggle="tooltip" data-placement="right" title="To add JVZoo Affiliate Products you must first get approved by the vendor of the product.">info</i></strong></h4>
        ...
        <table class="table">
            <tbody>
                ...
                <tr>
                    <th width="20%"></th>
                    <td>
                        @if($store->affiliateSettings->where('name', 'jvzoo')->first() !== null && json_decode($store->affiliateSettings->where('name', 'jvzoo')->first()->settings)->affiliate_id !== '')
                        <button type="button" class="btn btn-primary search"> Search </button>
                        @else
                        <button type="button" class="btn btn-primary search" disabled> Search </button> 
                        <p class="text-danger"><b>Please enter Affiliate ID for JVZoo Settings. Go to affiliate settings to add JVZoo Affiliate ID</b></p>
                        @endif
                    </th>
                </tr>
            </tbody>
...
@section('custom-scripts')
    ...
    <script>
        ...
        $('.search').on('click', function(){
            ...
            axios.post("{{ route('jvzoo.search', ['subdomain' => Session::get('subdomain')]) }}", {
                keyword: keyword,
            })
            .then(function (responses) {
                ...
                html += '    <th>Product Commision</th>'
                html += '    <th>Product Sales</th>'
                html += '    <th>Product Average Price</th>'
                html += '    <th>Sales Page</th>'
                ...
                $.each(responses.data, function(i, item){
                        ...
                        var data = {
                            title: escape(item.product_name),
                            image: image,
                            product_id: item.reference_id,
                            currency: 'USD',
                            price: 0.00,
                            details_link: item.affiliate_info_page,
                            sales_page: escape(item.sales_page),
                            description: '',
                            customer_review_link: '',
                            images: [],
                            permalink: createPermalink(item.product_name),
                        };
                        ...
                        html += "   <td><input class='product-data' type='hidden' value='" + data + "'>"+ item.product_name +"</td>";
                        html += '   <td>'+ item.vendor_name +'</td>';
                        html += '   <td>'+ item.product_commission +'</td>';
                        html += '   <td>'+ item.product_sales +'</td>';
                        html += '   <td>'+ item.product_average_price +'</td>';
                        html += '   <td><a href="'+ item.sales_page.replace('http://', 'https://') +'" target="_blank">View Page</a></td>';
                        html += '   <td><a href="'+ item.affiliate_info_page.replace('http://', 'https://') +'" target="_blank">View Details</a></td>';
