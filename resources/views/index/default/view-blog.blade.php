                        ...
                        <div class="col-lg-3 col-md-6 col-sm-12">
                            <div class="card border-primary mb-3">
                                <div class="card-header text-center">
                                    ...
                                    <span>
                                        @if(strcasecmp('amazon', $blogProduct->product->source) == 0)
                                            <img src="{{ asset('img/amazon-icon.png') }}" alt="{{ $blogProduct->product->source }}" width="50" class="img-fluid">
                                        @elseif(strcasecmp('aliexpress', $blogProduct->product->source) == 0)
                                            <img src="{{ asset('img/aliexpress-icon.png') }}" alt="{{ $blogProduct->product->source }}" width="50" class="img-fluid">
                                        @elseif(strcasecmp('ebay', $blogProduct->product->source) == 0)
                                            <img src="{{ asset('img/ebay-icon.png') }}" alt="{{ $blogProduct->product->source }}" width="50" class="img-fluid">
                                        @elseif(strcasecmp('walmart', $blogProduct->product->source) == 0)
                                            <img src="{{ asset('img/walmart-icon.jpg') }}" alt="{{ $blogProduct->product->source }}" width="50" class="img-fluid">
                                        @elseif(strcasecmp('shopcom', $blogProduct->product->source) == 0)
                                            <img src="{{ asset('img/shopcom-icon.png') }}" alt="{{ $blogProduct->product->source }}" width="100" class="img-fluid">
                                        @elseif(strcasecmp('cjcom', $blogProduct->product->source) == 0)
                                            <img src="{{ asset('img/cjcom-icon.png') }}" alt="{{ $blogProduct->product->source }}" width="40" class="img-fluid">
                                        @endif
                                    </span>
                                </div>
                                <div class="card-body product-content">
                                    ...
                                    {{ strlen(strip_tags($blogProduct->product->description)) > 90 
                                        ? html_entity_decode(substr(strip_tags(preg_replace('/<style>(.*?)<\/style>/s', '', html_entity_decode(htmlspecialchars_decode($blogProduct->product->description)))), 0, 87)) . "..." 
                                        : html_entity_decode(strip_tags(preg_replace('/<style>(.*?)<\/style>/s', '', html_entity_decode(htmlspecialchars_decode($blogProduct->product->description))))) }}
