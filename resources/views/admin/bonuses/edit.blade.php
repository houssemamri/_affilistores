...
<div class="card">
    <div class="card-header card-header-primary">
        <h4 class="card-title"><strong>Update Bonus </strong></h4>
        <p class="card-category">Update bonus details</p>
    </div>
    <div class="card-body">
        <form action="{{ route('bonuses.edit', $id) }}" method="POST" enctype="multipart/form-data"  id="bonusForm">
            {!! csrf_field() !!}
            <table class="table">
                <tbody>
                    <tr>
                        <th>Name</th>
                        <td>
                            <input type="text" name="name" value="{{ $bonus->name }}" class="form-control" required>
                        </td>
                    </tr>
                    <tr>
                        <th>Available for</th>
                        <td class="check">
                            ...
                            <div class="form-check">
                                <label class="form-check-label">
                                    <input class="form-check-input available" value="{{ $membership->id }}" name="available_for[]" type="checkbox" {{ $bonus->available->where('membership_id', $membership->id)->count() > 0 ? 'checked' : '' }}>
                                    {{ $membership->title }}
                                    <span class="form-check-sign">
                                        <span class="check"></span>
                                    </span>
                    ...
                    <tr>
                        <th>Description</th>
                        <td>
                            <textarea class="form-control" name="description" id="editor-description" required>{{ $bonus->description }}</textarea>
                        </td>
                    </tr>
                    <tr>
                        <th>Cover Image </th>
                        <td>
                            ...
                            <img src="{{ asset('bonuses/' . $bonus->image) }}" alt="" class="img-fluid">
                            <hr>
                            @endif
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="file" class="form-control-file" name="product_file" id="file" >
    ...
    <script>
        ...
        $('#bonusForm').submit(function(e){
            ...
            if(!description.summernote('code') || description.summernote('code') == '<p><br></p>')
                ...
                if($('input[name=name]').val() && description.summernote('code') && available > 0){
