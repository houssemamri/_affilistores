...
class JvzooGetProducts extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'product:jvzoo';
     ...
     * @var string
     */
    protected $description = 'Get products from JVZoo';
    ...
    public function handle(){
        ...
        foreach ($automations as $automation) {
            ...
            if(!$this->checkFeatures($automation->store, 'jvzoo')) continue;    
    ...
    public function search($automation)
    {
        ...
        $items = JvzooProduct::where('product_name', 'LIKE', '%' . $keyword . '%')->take(1000)->get();
        ...
        foreach ($items as $item) {
            ...
            if($is_already_existing == 0){
                $product_details = [
                    ...
                    'description' => '',
                    'permalink' => $this->clean($item->product_name),
    ...
    public function insertNewProducts($automation, $items, $product_data, $store){
        foreach ($items as $item) {
            $item['details_link'] = 'https://jvz1.com/c/' . json_decode($store->affiliateSettings->where('name', 'jvzoo')->first()->settings)->affiliate_id . '/' . $item['reference_id'] ;
