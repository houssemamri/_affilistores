...
class WarriorPlusGetProducts extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'product:warriorplus';
     ...
     * @var string
     */
    protected $description = 'Get products from Warrior Plus';
    ...
    public function handle(){
        $automations = Automation::whereDate('start_date', '<=', date('Y-m-d'))
            ...
            ->where('source', 'warriorplus')
            ->get();

        foreach ($automations as $automation) {
            if(!isset($automation->store)) continue;
         
            if(!$this->checkFeatures($automation->store, 'warriorplus')) continue;
    ...
    public function search($automation)
    {
        ...
        $items = WarriorPlusProduct::where('offer_name', 'LIKE', '%' . $keyword . '%')->take(1000)->get();
    ...
    public function validateProducts($items, $store, $limit, $product_data){
        ...
        foreach ($items as $item) {
            ...
            if($is_already_existing == 0){
                $product_details = [
                    'reference_id' => $item->offer_code,
                    'name' => $item->offer_name,
                    'description' => '',
                    'permalink' => $this->clean($item->offer_name),
