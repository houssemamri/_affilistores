...
class ClickBankGetProducts extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'product:clickbank';
     ...
     * @var string
     */
    protected $description = 'Get products from ClickBank';
    ...
    public function handle(){
        $automations = Automation::whereDate('start_date', '<=', date('Y-m-d'))
            ...
            ->where('source', 'clickbank')
            ->get();

        foreach ($automations as $automation) {
            if(isset($automation->store)){
    ...
    public function search($automation)
    {
        ...
        $items = ClickBankProduct::orWhere('title', 'LIKE', '%' . $keyword . '%')->orWhere('description', 'LIKE', '%' . $keyword . '%')->take(1000)->get();
        ...
        foreach ($items as $item) {
            ...
            if($is_already_existing == 0){
                $product_details = [
                    ...
                    'permalink' => $this->clean($item->title),
                    'details_link' => '',
    ...
    public function insertNewProducts($automation, $items, $product_data, $store){
        foreach ($items as $item) {
            $item['details_link'] = 'http://' . json_decode($store->affiliateSettings->where('name', 'clickbank')->first()->settings)->account_name . '.' . $item['reference_id'] .'.hop.clickbank.net';
